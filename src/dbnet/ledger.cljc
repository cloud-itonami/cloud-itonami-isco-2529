(ns dbnet.ledger
  "Append-only audit ledger for the ISCO-08 2529 database and network
  professionals (NEC) actor: entry construction, hash chaining, and
  verification.
  ...
  audit is trying to check. For a record of which schema migrations were
  accepted as normalizing, that is the difference between an audit that can
  show which migrations were approved and one that can only show which
  migrations someone chose to leave in the file.
  ...
  a production `:apply-migration` committed through human sign-off and a
  routine `:propose-migration` committed automatically both produced
  `{:disposition :commit ...}` with no field distinguishing them.
  `commit-entry` therefore takes the approval provenance as part of the
...
(defn summary
  "One line per entry, for operators and for `dbnet.sim` output."
  [ledger]
  (apply str
         (interpose "\n"
                    (for [e ledger]
